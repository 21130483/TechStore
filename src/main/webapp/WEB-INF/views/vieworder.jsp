                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex align-items-center">
                                <h4 class="card-title">Chi tiết đơn hàng #${purchase.purchaseID}</h4>
                        ...
                        <div class="card-body">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <table class="table">
                                        ...
                                        <tr>
                                            <th>Trạng thái:</th>
                                            <td>
                                                <span class="badge badge-${purchase.status == 'DELIVERED' ? 'success' : 
                                                                    purchase.status == 'CANCELLED' ? 'danger' : 
                                                                    purchase.status == 'PROCESSING' ? 'warning' : 'info'}">
                                                    ${purchase.status}
                                                </span>
                                            </td>
                                        </tr>
                                    ...
                                    <table class="table">
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th><fmt:formatNumber value="${purchase.totalPrice}" type="currency" currencySymbol="₫"/></th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
                            ...
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    ...
                                    <p>${purchase.shippingAddress}</p>
                                </div>
                            </div>

                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <form action="<c:url value='/admin/orders/update-status/${purchase.purchaseID}'/>" method="POST" class="form-inline">
                                        <div class="form-group mr-3">
                                            <label for="status" class="mr-2">Cập nhật trạng thái:</label>
                                            <select name="status" id="status" class="form-control" onchange="this.form.submit()">
                                                <option value="PENDING" ${purchase.status == 'PENDING' ? 'selected' : ''}>Chờ xử lý</option>
                                                <option value="PROCESSING" ${purchase.status == 'PROCESSING' ? 'selected' : ''}>Đang xử lý</option>
                                                <option value="SHIPPED" ${purchase.status == 'SHIPPED' ? 'selected' : ''}>Đang giao</option>
                                                <option value="DELIVERED" ${purchase.status == 'DELIVERED' ? 'selected' : ''}>Đã giao</option>
                                                <option value="CANCELLED" ${purchase.status == 'CANCELLED' ? 'selected' : ''}>Đã hủy</option>
                                            </select>
                                        </div>
                                    </form>
                                </div>
